...
export default function SlabHistory({ history, isLoading = false }: SlabHistoryProps) {
  ...
  if (isLoading) {
    return (
      <Card className="w-full max-w-3xl mx-auto border-white/5 bg-slate-950/40 backdrop-blur-md shadow-2xl mt-6">
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto border-white/[0.06] shadow-2xl bg-slate-950/40 backdrop-blur-md overflow-hidden mt-6 relative">
      ...
      <CardContent className="pb-6">
        {history.length === 0 ? (
          <div className="text-center py-12 text-slate-500 border border-dashed border-white/[0.06] rounded-2xl bg-white/[0.01] flex flex-col items-center justify-center">
            <History className="h-10 w-10 stroke-1 mb-3 text-slate-650" />
          ...
          <motion.div
            ...
            initial="hidden"
            animate="show"
            className="relative border-l border-white/[0.08] pl-4 sm:pl-6 space-y-4 ml-3"
          >
            {history.map((log) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="bg-[#10101a] border border-white/[0.04] hover:border-white/[0.1] rounded-xl hover:shadow-[0_0_20px_rgba(200,16,46,0.04)] transition-all duration-200 overflow-hidden">
                    {/* Header bar */}
                    <div
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-slate-400">
                          ...
                          <span className="truncate max-w-[160px] font-medium" title={log.changedBy}>
                            {log.changedBy}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2.5">
                        <span className="text-[9px] font-black uppercase tracking-wider text-red-400 bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20 font-mono tabular-nums">
                    ...
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        <motion.div
                          ...
                          className="border-t border-white/[0.06] bg-[#080810]/60 px-4 py-4"
                        >
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                            {slabs.map((slab: any, idx: number) => (
                              <div
                                key={idx}
                                className="flex items-center justify-between bg-[#10101a] border border-white/[0.04] p-3 rounded-lg text-xs"
                              >
                                <span className="font-extrabold text-slate-300 font-mono tabular-nums">
                                  {slab.maxUnits === null
                                    ? `${slab.minUnits}+`
