  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-black text-slate-900 dark:text-white tracking-widest uppercase font-heading">
        ...
        <Button
          ...
          className="border-slate-200 dark:border-white/[0.08] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-50 dark:bg-slate-950/40 font-black text-xs uppercase tracking-wider rounded-xl py-4 active:scale-95 transition-all"
        >
      ...
      {submissions.length === 0 ? (
        <Card className="border-border bg-card/40 backdrop-blur-md shadow-2xl">
          <CardContent className="py-16 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-sm font-black text-slate-900 dark:text-white tracking-wider uppercase mb-1">
              Ledger Clear
            </h3>
        ...
        <div className="grid grid-cols-1 gap-4">
          {submissions.map((group) => {
            ...
            return (
              <Card
                key={cardKey}
                className={`border-border bg-card/40 backdrop-blur-md shadow-xl overflow-hidden transition-all duration-300 ${
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="min-w-0 space-y-0.5">
                      <div className="flex items-center flex-wrap gap-2">
                        <span className="text-sm font-black text-slate-800 dark:text-white font-heading uppercase tracking-wide truncate">
                          {group.userName}
                        </span>
                        
                        <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/[0.05] text-slate-600 dark:text-slate-350 px-2 py-0.5 rounded-md font-mono">
                          <Calendar className="h-3 w-3 text-red-500" />
                          <span>{periodLabel}</span>
                        </div>
                      </div>
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate block font-mono">
                        {group.userEmail}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 border-slate-200 dark:border-white/[0.04] pt-3 md:pt-0">
                    <div className="text-left md:text-right">
                      <span className="text-[9px] text-slate-400 dark:text-slate-500 font-bold block uppercase tracking-wider leading-none mb-1">Volume Registered</span>
                      <span className="text-xs font-black font-mono text-slate-800 dark:text-slate-200 tabular-nums">
                        {totalUnits} units
                      </span>
                    </div>
                    
                    <div className="text-left md:text-right border-l border-slate-200 dark:border-white/[0.08] pl-5">
                      <span className="text-[9px] text-slate-400 dark:text-slate-500 font-bold block uppercase tracking-wider leading-none mb-1">Calculated Incentive</span>
                      <span className="text-xs font-black font-mono text-emerald-600 dark:text-emerald-450 tabular-nums">
                    ...
                    <div className="flex items-center gap-2 border-l border-white/[0.08] pl-5 shrink-0">
                      {resolvedInfo ? (
                        ...
                        <div className="text-slate-400 dark:text-slate-500 hover:text-slate-800 dark:hover:text-white transition-colors">
                ...
                {resolvedInfo && (
                  <div className="border-t border-slate-200 dark:border-white/[0.04] bg-slate-50 dark:bg-white/[0.01] px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fade-in">
                    ...
                    <div className="text-[10px] text-slate-500 dark:text-slate-500 font-mono font-bold tracking-wide">
                ...
                {isExpanded && !resolvedInfo && (
                  <CardContent className="px-5 pb-5 pt-0 border-t border-slate-200 dark:border-white/[0.04] bg-slate-50/50 dark:bg-[#0c0c14]/40">
                    <div className="py-4 space-y-4">
                      ...
                      <div className="divide-y divide-slate-100 dark:divide-white/[0.04] bg-white dark:bg-slate-950/30 border border-slate-200 dark:border-white/[0.04] rounded-xl px-4">
                          ...
                          return (
                            <div key={rec.carModelId} className="py-3.5 flex items-center justify-between gap-4">
                              <div className="min-w-0">
                                <span className="text-xs font-extrabold text-slate-800 dark:text-[#eeeff7] uppercase tracking-wide block truncate">
                                  {rec.carModelName}
                                </span>
                                <span className="text-[10px] text-slate-500 dark:text-slate-450 font-bold block tracking-wider font-mono">
                                  {rec.carVariant}
                                </span>
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                <Button
                                  ...
                                  disabled={qty <= 0}
                                  className="w-7 h-7 bg-slate-100 dark:bg-white/5 hover:bg-red-500/10 hover:text-red-400 rounded-lg border border-slate-200 dark:border-white/[0.04] hover:border-red-500/20 active:scale-90 transition-all font-mono font-bold text-xs text-slate-700 dark:text-slate-300"
                                >
                                  -
                                </Button>
                                <Input
                                  ...
                                  className="w-12 h-7 p-0 text-center font-mono font-black text-xs bg-white dark:bg-slate-900 border-slate-200 dark:border-white/[0.08] text-slate-800 dark:text-slate-200 focus:border-red-500 tabular-nums"
                                />
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => handleQtyChange(group.userId, group.month, group.year, rec.carModelId, qty + 1)}
                                  className="w-7 h-7 bg-slate-100 dark:bg-white/5 hover:bg-indigo-500/10 hover:text-indigo-400 rounded-lg border border-slate-200 dark:border-white/[0.04] hover:border-indigo-500/20 active:scale-90 transition-all font-mono font-bold text-xs text-slate-700 dark:text-slate-300"
                                >
                                  +
                                </Button>
                              </div>
