...
export default function AdminPortal() {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-450 tracking-widest">
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans antialiased select-none relative">
      <Toaster position="top-right" richColors closeButton />
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 hidden sm:block">
              <span className="font-black text-slate-900 dark:text-white tracking-widest text-xs block font-heading leading-none">
                NIPPON TOYOTA
              </span>
              <span className="text-[8px] text-slate-500 dark:text-slate-400 font-extrabold tracking-widest uppercase block mt-1">
                Admin Configuration
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5 px-3.5 py-1.5 bg-slate-100/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.04] rounded-xl shadow-lg ring-1 ring-black/[0.01] dark:ring-white/[0.02]">
            ...
            <motion.button
              ...
              onClick={logout}
              className="flex items-center justify-center w-8 h-8 text-slate-400 dark:text-slate-500 hover:text-[#c8102e] hover:bg-slate-100 dark:hover:bg-white/[0.02] border border-slate-200 dark:border-white/[0.04] hover:border-red-500/20 rounded-xl transition-all duration-300 shadow-md cursor-pointer shrink-0"
              title="Logout"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-16 space-y-6">
        ...
        <motion.div
          ...
          className="flex items-start gap-3 border-b border-slate-200 dark:border-white/[0.04] pb-5"
        >
          <div className="w-1 h-8 bg-primary rounded-full shrink-0" />
          <div className="space-y-1">
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none font-heading uppercase">
              Configuration Panel
            </h1>
            <p className="text-xs text-slate-550 dark:text-slate-400 font-semibold max-w-lg">
            ...
            return (
              <button
                ...
                className={`relative group flex items-center gap-3 px-4 py-3 rounded-2xl text-xs sm:text-sm font-semibold transition-all duration-300 w-full sm:w-auto justify-start border cursor-pointer ${
                  isActive
                    ? "text-slate-900 dark:text-white border-slate-350 dark:border-white/[0.08]"
                    : "text-[#85879b] border-slate-200 dark:border-white/[0.02] bg-slate-50 dark:bg-white/[0.01] hover:bg-slate-100 dark:hover:bg-white/[0.03] hover:text-slate-900 dark:hover:text-white"
                }`}
              >
                {isActive && (
                  <motion.div
                    layoutId="admin-active-tab"
                    className="absolute inset-0 bg-slate-200/50 dark:bg-white/[0.03] rounded-2xl shadow-xl ring-1 ring-slate-300 dark:ring-white/[0.06]"
                ...
                <span className="relative z-10 flex items-center gap-3 w-full">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-300 shrink-0 border ${
                    isActive
                      ...
                      : "bg-slate-100 dark:bg-[#080810] border-slate-200 dark:border-white/[0.04] text-slate-400 dark:text-slate-500 group-hover:text-slate-900 dark:group-hover:text-white"
                  }`}>
                    <Icon className="h-4 w-4 shrink-0" />
                  </div>
                  <div className="text-left hidden sm:block min-w-0">
                    <div className="text-xs font-black tracking-wide leading-none font-heading uppercase text-slate-800 dark:text-white">
                      {tab.label}
                    </div>
                    <div className="text-[8px] text-slate-500 dark:text-slate-500 font-extrabold uppercase mt-1 leading-none tracking-wider truncate">
                      {tab.description}
                    </div>
                  </div>
            ...
            {activeTab === "inventory" ? (
              <div className="bg-card border border-border rounded-2xl p-6 shadow-2xl relative">
                <div className="mb-6 flex items-center gap-3 pb-4 border-b border-border">
                  ...
                  <div>
                    <h2 className="text-sm font-black text-slate-900 dark:text-white font-heading uppercase leading-none">
